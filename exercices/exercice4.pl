...
% FOREACH
    genSet([X|L], Result, BuildedSet) :-
        member(X, BuildedSet) -> genSet(L, Result, BuildedSet) ; genSet(L, Result, [X|BuildedSet]).
...
% all following function are assuming the parameters are (un)ordered sets!!!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% arg3 = arg1 U arg2
% END
...
% FOREACH
    setUnion([X|A], Union, Result) :-
        member(X,Union) -> setUnion(A,Union,Result) ; setUnion(A, [X|Union], Result).
...
% FOREACH
    setInter([X|A], B, Result, Inter) :-
        member(X,B) -> setInter(A, B, Result, [X|Inter]) ; setInter(A, B, Result, Inter).
...
% FOREACH
    setMinus([X|A], B, Result, Inter) :-
        member(X,B) -> setMinus(A, B, Result, Inter) ; setMinus(A, B, Result, [X|Inter]).
...
% Actualy, the areEqualSets time complexity is the same as setMinus
% O(len(A)*len(B)). But with sorted sets given into parameters, this
% complexity would fall down to O(min(len(A),len(B))).
